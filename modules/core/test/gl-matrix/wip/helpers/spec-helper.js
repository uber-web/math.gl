...
global.expect = function (e) {
  ...
  return {
    ...
    toBeEqualish: function (a) {
      if (typeof e == 'number') {
        if (isNaN(e) !== isNaN(a)) expected(e, 'to be equalish to', a);
        if (Math.abs(e - a) >= EPSILON) expected(e, 'to be equalish to', a);
      }

      if (e.length != a.length) assert.fail(e.length, a.length, 'length mismatch');

      for (let i = 0; i < e.length; i++) {
        if (isNaN(e[i]) !== isNaN(a[i])) assert.fail(isNaN(e[i]), isNaN(a[i]));
        if (Math.abs(e[i] - a[i]) >= EPSILON) assert.fail(Math.abs(e[i] - a[i]));
      }
    },
    ...
    toBeEqualishQuat2: function (a, epsilon) {
      ...
      if (e.length != a.length) expected(e, 'to have the same length as', a);

      for (let i = 0; i < e.length; i++) {
        if (isNaN(e[i]) !== isNaN(a[i])) expected(e, 'to be equalish to', a);

        if (allSignsFlipped) {
          if (Math.abs(e[i] - -a[i]) >= epsilon) expected(e, 'to be equalish to', a);
        } else {
